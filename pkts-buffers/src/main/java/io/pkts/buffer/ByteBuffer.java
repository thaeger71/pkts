...
public final class ByteBuffer extends AbstractBuffer {
	...
	protected ByteBuffer(final int readerIndex, final int lowerBoundary, final int upperBoundary, final int writerIndex,
			final byte[] buffer) {
	...
	public int getInt(final int index) {
		...
		if (this.byteOrder.equals(ByteOrder.BIG_ENDIAN)) {
			return (this.buffer[i] & 0xff) << 24 | (this.buffer[i + 1] & 0xff) << 16 | (this.buffer[i + 2] & 0xff) << 8
					| (this.buffer[i + 3] & 0xff) << 0;
		} else {
			return (this.buffer[i] & 0xff) << 0 | (this.buffer[i + 1] & 0xff) << 8 | (this.buffer[i + 2] & 0xff) << 16
					| (this.buffer[i + 3] & 0xff) << 24;
		}
	}
	...
	public void write(final String s)
			throws IndexOutOfBoundsException, WriteNotSupportedException, UnsupportedEncodingException {
		write(s, "UTF-8");
	}

	@Override
	public void write(final String s, final String charset)
			throws IndexOutOfBoundsException, WriteNotSupportedException, UnsupportedEncodingException {
		final byte[] bytes = s.getBytes(charset);
		if (!checkWritableBytesSafe(bytes.length)) {
			throw new IndexOutOfBoundsException(
					"Unable to write the entire String to this buffer. Nothing was written");
		}
	...
	public void write(final int value) throws IndexOutOfBoundsException, WriteNotSupportedException {
		...
		if (this.byteOrder.equals(ByteOrder.BIG_ENDIAN)) {
			...
			this.buffer[index + 0] = (byte) value;
	...
	public ByteOrder order() {
  	...
  	public Buffer order(ByteOrder bo) {
    	this.byteOrder = bo != null ? bo : ByteOrder.LITTLE_ENDIAN;
  		return this;
